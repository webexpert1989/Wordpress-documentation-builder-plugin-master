...
/**************************************************************
 *
 * talbe class for documentation list admin page 
 *
 * @package  		 TRUEWordpress Documentation Builder Plugin
 * @Version			 1.0.1
 * @file	 		 inc/class-list.php
...
if(!class_exists('DB_admin_list')){
	// CREATE A PACKAGE CLASS
	class DB_admin_list extends WP_List_Table{
		...
		function __construct($table_rows = array(), $total = 0){
			...
			parent::__construct(array(
				'singular'  => 'doc',		//singular name of the listed records
				'plural'	=> 'docs',		//plural name of the listed records
			...
			$this->rows_per_page = $DB_settings['rows_per_page'];
		}
		 ...
		 */
		function column_doc_name($item){
			
			//Build row actions
			$actions = array(
				'edit'		=> sprintf('<a href = "?page=%s&action=%s&doc=%s">'.__('Edit', 'truewordpress').'</a>', $_REQUEST['page'], 'edit', $item->ID),
				'export'	=> sprintf('<a href = "?page=%s&action=%s&doc=%s">'.__('Export', 'truewordpress').'</a>', $_REQUEST['page'], 'export', $item->ID),
				'delete'	=> sprintf('<a href = "#" onclick = "javascript: removeDocOfList(\''.__('Do you remove this documentation really?', 'truewordpress').'\', \'?page=%s&action=%s&doc=%s\');">'.__('Remove', 'truewordpress').'</a>', $_REQUEST['page'], 'del', $item->ID),
			);
		...
		function column_cb($item){
			return sprintf(
				...
				/*$1%s*/ $this->_args['singular'],  //Let's simply repurpose the table's singular label("doc")
		...
		function get_columns(){
			$columns = array(
				...
				'doc_name'		=> __('Title', 'truewordpress'),
				'author'		=> __('Author', 'truewordpress'),
				'doc_shortcode' => __('Shortcode', 'truewordpress'),
				'date'			=> __('Date', 'truewordpress')
			);
			return $columns;
		}
		...
		function get_sortable_columns(){
			$sortable_columns = array(
				'doc_name'	 => array('doc_name', false),	 //true means it's already sorted
